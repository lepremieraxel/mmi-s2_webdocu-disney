<?php
  ...
  if(isset($_POST['q1']) && isset($_POST['q2']) && isset($_POST['q3']) && isset($_POST['q4']) && isset($_POST['q5']) && isset($_POST['q6']))
  {
    ...
    $check = $bdd->prepare('SELECT * FROM q_cultureg WHERE token_user = ?');
    ...
    if($row == 0)
    {
      ...
      $insert = $bdd->prepare('INSERT INTO q_cultureg(token_user, token, score) VALUES(:token_user, :token, :score)');
      $insert->execute(array(
        'token_user' => $_SESSION['user'],
        'token' => bin2hex(openssl_random_pseudo_bytes(64)),
        'score' => $score
      ));
      header('Location:../index.php?game_err=success&s='.$score); die();
    }else header('Location:index.php?quizz_err=already'); die();
  }else header('Location:index.php?quizz_err=empty'); die();