...
session_start();
    ...
    if(isset($_POST['s1']) && isset($_POST['s2']) && isset($_POST['s3']) && isset($_POST['s4']) && isset($_POST['s5']) && isset($_POST['s6']) && isset($_POST['s7']) && isset($_POST['s8']) && isset($_POST['s9']))
    {
        ...
        if($row == 0)
        {
            $insert = $bdd->prepare('INSERT INTO s_perso(token_user, token, s1, s2, s3, s4, s5, s6, s7, s8, s9) VALUES(:token_user, :token, :s1, :s2, :s3, :s4, :s5, :s6, :s7, :s8, :s9)');
            ...
            $update = $bdd->prepare('UPDATE s_perso SET s1 = ?, s2 = ?, s3 = ?, s4 = ?, s5 = ?, s6 = ?, s7 = ?, s8 = ?, s9 = ? WHERE token_user = ?');
